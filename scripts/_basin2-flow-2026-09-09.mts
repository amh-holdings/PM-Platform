/** Basin 2 ESC (5.1.1.7) gets Basin 1's flow. Culvert outflow waits on Basin 1
 *  being complete - all three of its chain-ends, not just the latest, so a slip
 *  in the riser or spillway still holds Basin 2 back.
 *  Then the same forward pass, then span summaries. --apply to write. */
...
// Basin 1's flow, transposed onto Basin 2. Culvert's predecessor is Basin 1 complete.
const B1_DONE="5.1.1.6.7, 5.1.1.6.4, 5.1.1.6.2";
const FLOW:Record<string,string>={
 "5.1.1.7.1":B1_DONE,          // Culvert outflow      <- Basin 1 complete
 "5.1.1.7.2":"5.1.1.7.1SS",    // Riser install        <- alongside culvert
 "5.1.1.7.5":"5.1.1.7.1",      // Embankment           <- culvert
 "5.1.1.7.4":"5.1.1.7.5",      // Emergency spillway   <- embankment
 "5.1.1.7.3":"5.1.1.7.5",      // Lined inlets install <- embankment
 "5.1.1.7.6":"5.1.1.7.3",      // Diversion ditch line <- lined inlets
 "5.1.1.7.7":"5.1.1.7.6, 5.1.1.7.5", // Matting        <- diversion + embankment
};

const {data}=await sb.from("schedule_tasks").select("*").eq("project_id",PID).order("sort_order");
const all=(data as any[]).map(t=>({...t}));
writeFileSync(`scripts/_backups/basin2-flow-pre-${TODAY}.json`,JSON.stringify(data,null,2));
const orig=new Map((data as any[]).map(t=>[t.wbs_code,t]));

console.log("=== STEP 1: Basin 2 flow ===");
for(const t of all){ const f=FLOW[t.wbs_code]; if(f===undefined) continue;
  console.log(`  ${t.wbs_code} ${String(t.task_name).padEnd(24)} pred: ${String(t.predecessors??"-").padEnd(20)} -> ${f}`);
  t.predecessors=f; }
...
if(order.length!==leaves.length) throw new Error("CYCLE: "+leaves.filter(t=>!order.includes(t.wbs_code)).map(t=>t.wbs_code).join(", "));
...
for(const wbs of order){
  ...
  if(started){ let e=t.end_date;
    if(dep){const rem=Math.max(1,Math.ceil(dur*(1-pct/100)));const f=endOf(mx(dep,workStart),rem);if(pd(f)>pd(e))e=f;}
...
for(const t of all){ if(!S.has(t.wbs_code))continue;
  ...
  if(o.predecessors!==t.predecessors)ch.push(`pred -> ${t.predecessors}`);
  if(ch.length)moves.push({t,s,e,dur,ch}); }
for(const m of moves)console.log(`  ${m.t.wbs_code.padEnd(11)}${isSum(m.t)?"[S]":"   "} ${String(m.t.task_name).slice(0,30).padEnd(30)} ${m.ch.join(" | ")}`);
console.log("\nBASIN 2 ESC RESULT:");
for(const w of ["5.1.1.7.1","5.1.1.7.2","5.1.1.7.5","5.1.1.7.4","5.1.1.7.3","5.1.1.7.6","5.1.1.7.7"])
  console.log(`  ${w} ${String(byW.get(w)!.task_name).padEnd(24)} d${byW.get(w)!.duration_days} ${S.get(w)}..${E.get(w)}`);
...
console.log(`\n${moves.length} rows move. Basin 1 done ${E.get("5.1.1.6")} -> Basin 2 starts ${S.get("5.1.1.7")}`);
console.log(`Phase 1 ${E.get("5.1.1")} | Phase 2 starts ${p2} | ordered: ${pd(p2!)>pd(p1!)}`);
...
if(APPLY){for(const m of moves){const p:any={start_date:m.s,end_date:m.e,duration_days:m.dur};
  if(FLOW[m.t.wbs_code]!==undefined)p.predecessors=m.t.predecessors;

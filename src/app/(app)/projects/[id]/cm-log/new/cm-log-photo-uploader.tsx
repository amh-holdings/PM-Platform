...
// Reuse the existing private DPR photo bucket; CM-log photos live under a
// cm-logs prefix. The server records the metadata row on submit.
const CM_LOG_PHOTO_BUCKET = "dpr-photos";
const MAX_PHOTO_BYTES = 15 * 1024 * 1024; // 15 MB

export type StagedCmPhoto = {
  photoId: string;
  fileName: string;
  storagePath: string;
  caption: string;
...
type Props = {
  projectId: string;
  draftId: string;
  photos: StagedCmPhoto[];
  onChange: (photos: StagedCmPhoto[]) => void;
};
...
export function CmLogPhotoUploader({
  projectId,
  draftId,
  photos,
  onChange,
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      const storagePath = `${projectId}/cm-logs/_drafts/${draftId}/${photoId}-${sanitizeFileName(file.name)}`;
  ...
  const removePhoto = useCallback(
    async (photoId: string) => {
      ...
      const supabase = createClient();
      await supabase.storage.from(CM_LOG_PHOTO_BUCKET).remove([photo.storagePath]);
    },
    [photos, onChange],
  );

  function patchPhoto(photoId: string, patch: Partial<StagedCmPhoto>) {
